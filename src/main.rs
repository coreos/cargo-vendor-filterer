...
/// Key in cargo metadata for dependencies.
const DEPENDENCIES: &str = "dependencies";
/// Key in cargo metadata for dependencies only necessary when testing the crate.
const DEV_DEPENDENCIES: &str = "dev-dependencies";
/// Key in cargo metadata for dependencies only necessary when building the crate.
...
fn filter_path_dependencies(t: &toml::value::Table) -> Option<toml::value::Table> {
...
fn filter_path_dependencies_from_manifest(manifest: toml::Value) -> Result<Option<toml::Value>> {
    let t = if let toml::Value::Table(t) = manifest {
        t
    } else {
        return Ok(None);
    };
    let t = t
        ...
        .filter_map(|(k, v)| {
            match (k.as_str(), v.as_table()) {
                (DEPENDENCIES | DEV_DEPENDENCIES | BUILD_DEPENDENCIES, Some(ref t)) => {
                    let v = filter_path_dependencies(t);
                    return v.map(|v| (k.to_string(), toml::Value::Table(v)));
                }
                _ => {}
            }
            Some((k, v))
        ...
        //.map(|(k, v)| match (k, v) {
        //    (k, toml::Value::Table(features)) if k == FEATURES => {
        //        let features = features
        //            .into_iter()
        //            .map(|v| (v.0, toml::Value::Array(Vec::new())))
        //            .collect();
        //        (k, toml::Value::Table(features))
        //    }
        //    o => o,
        //})
        .collect();
...
/// Remove build and dev dependencies, since in some crates these have `path=` referneces
/// which can't be resolved.  We also have to neuter the features sets
/// refer to these.
/// See https://github.com/coreos/cargo-vendor-filterer/issues/32
fn neuter_dev_dependencies_and_feautres(path: &Utf8Path) -> Result<()> {
    let cargo_path = &path.join("Cargo.toml");
    let cargo_toml =
        std::fs::read_to_string(cargo_path).with_context(|| format!("Writing {cargo_path}"))?;
    let v: toml::Value =
        toml::from_str(&cargo_toml).with_context(|| format!("Parsing {cargo_path}"))?;

    if let Some(v) = filter_path_dependencies_from_manifest(v)? {
        let v = toml::to_string(&v).context("Reserializing manifest")?;
        std::fs::write(&cargo_path, &v).with_context(|| format!("Writing {cargo_path}"))?;
    } else {
        eprintln!("warning: {cargo_path} is not a TOML table");
    }
...
fn replace_with_stub(path: &Utf8Path, offline: bool) -> Result<()> {
    ...
    neuter_dev_dependencies_and_feautres(path).context("Removing dev-dependencies")?;
...
"#,
    ...
    .unwrap();
    {
        ...
        assert!(t.contains_key(BUILD_DEPENDENCIES));
    }
    let v = filter_path_dependencies_from_manifest(v).unwrap();
    let filtered = v.expect("filtered value");
    let t = filtered.as_table().unwrap();
    let build_deps = t.get(BUILD_DEPENDENCIES).unwrap().as_table().unwrap();
    assert_eq!(build_deps.len(), 1);
    assert!(build_deps.get("foo").is_some());
    assert!(build_deps.get("winapi-build").is_none());
}
