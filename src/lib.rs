...
/// The key set to true in `package.metadata.vendor-filter` of the manifest of a
/// package replaced with a stub
pub const STUB_KEY: &str = "stub";
/// The table in `package.metadata.vendor-filter` of the manifest of a stub
/// which holds the keys removed from its package section
pub const STUB_REMOVED_PACKAGE_KEYS: &str = "removed-package-keys";
/// The manifest as written by the developer, which `cargo vendor` copies from
/// the published package next to the manifest cargo normalized
pub const CARGO_TOML_ORIG: &str = "Cargo.toml.orig";
/// The name of the file a stub keeps the manifest it was generated from in,
/// the one `cargo vendor` wrote before it was filtered
pub const CARGO_TOML_PRE_VENDOR_FILTER: &str = "Cargo.toml.pre-vendor-filter";

/// The path to the stub library file we write
const STUB_LIBRS: &str = "src/lib.rs";

/// The comment at the top of the manifest of a stub
const STUB_HEADER: &str = "\
# Generated by cargo-vendor-filterer: this package is not used by the vendored
# configuration and was replaced with a stub. The keys removed from it are
# listed in its metadata, and the manifest it was generated from is kept in
# Cargo.toml.pre-vendor-filter.
";
...
/// Steps:
///
/// - Remove everything except the manifests
...
fn replace_with_stub(path: &Utf8Path) -> Result<Vec<(&'static str, toml::Value)>> {
    ...
    let developer_manifest = match std::fs::read(path.join(CARGO_TOML_ORIG)) {
        Ok(data) => Some(data),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => None,
        Err(e) => return Err(e).with_context(|| format!("Reading {CARGO_TOML_ORIG}")),
    };
    ...
    // An empty Cargo.toml, saying where it comes from
    let cargo_toml_data = format!("{STUB_HEADER}{cargo_toml_data}");
    writef(Utf8Path::new(CARGO_TOML), cargo_toml_data.as_bytes())?;
    if let Some(data) = developer_manifest.as_deref() {
        writef(Utf8Path::new(CARGO_TOML_ORIG), data)?;
    }
    // The manifest it was generated from, for tools which need what was filtered out
    writef(
        Utf8Path::new(CARGO_TOML_PRE_VENDOR_FILTER),
...
#[test]
fn test_filter_manifest_stub_marker() {
    let mut v: toml::Value = toml::from_str(
        r#"
[package]
name = "openssl-sys"
...
"#,
    ...
    let removed = metadata[CONFIG_KEY][STUB_REMOVED_PACKAGE_KEYS]
        .as_table()
        .unwrap();
    assert_eq!(removed.len(), 2);
    assert_eq!(removed["links"].as_str(), Some("openssl"));
    assert_eq!(removed["build"].as_str(), Some("build/main.rs"));
    // Metadata the package already had is kept.
    assert!(metadata["docs"]["rs"].get("targets").is_some());
...
#[test]
fn test_replace_with_stub() {
    for with_orig in [true, false] {
        let temp_dir = tempfile::TempDir::new().unwrap();
        let path = Utf8Path::from_path(temp_dir.path()).unwrap();
        std::fs::create_dir_all(path.join("src")).unwrap();
        let manifest = "[package]\nname = \"foo\"\nversion = \"1.0.0\"\nlinks = \"foo\"\n";
        std::fs::write(path.join(CARGO_TOML), manifest).unwrap();
        std::fs::write(path.join(STUB_LIBRS), "fn f() {}").unwrap();
        std::fs::write(path.join("README.md"), "# foo").unwrap();
        if with_orig {
            std::fs::write(path.join(CARGO_TOML_ORIG), "[package]\nname = \"foo\"\n").unwrap();
        }
        std::fs::write(
            path.join(CARGO_CHECKSUM),
            r#"{"files":{"README.md":"00"},"package":"cafe"}"#,
        )
        .unwrap();

        let removed = replace_with_stub(path).unwrap();
        assert_eq!(removed.len(), 1);
        assert_eq!(removed[0].0, "links");

        // The manifest cargo vendored is kept as it was, and so is the one the
        // developer wrote, when the package carries it.
        assert_eq!(
            std::fs::read_to_string(path.join(CARGO_TOML_PRE_VENDOR_FILTER)).unwrap(),
            manifest
        );
        assert_eq!(
            path.join(CARGO_TOML_ORIG).exists(),
            with_orig,
            "{CARGO_TOML_ORIG} was handled wrongly"
        );
        // Everything else is gone, and the stub says what it is.
        assert!(!path.join("README.md").exists());
        assert_eq!(std::fs::read(path.join(STUB_LIBRS)).unwrap(), b"");
        let stub = std::fs::read_to_string(path.join(CARGO_TOML)).unwrap();
        assert!(stub.starts_with(STUB_HEADER), "{stub}");
        let stub: toml::Value = toml::from_str(&stub).unwrap();
        let package = stub[MANIFEST_KEY_PACKAGE].as_table().unwrap();
        assert!(!package.contains_key("links"));
        assert_eq!(
            package["metadata"][CONFIG_KEY][STUB_REMOVED_PACKAGE_KEYS]["links"].as_str(),
            Some("foo")
        );

        // Every file of the stub is checksummed, and the package checksum stays.
        let checksums: CargoChecksums =
            serde_json::from_reader(std::fs::File::open(path.join(CARGO_CHECKSUM)).unwrap())
                .unwrap();
        assert_eq!(checksums.package.as_deref(), Some("cafe"));
        let mut files: Vec<_> = checksums.files.keys().map(String::as_str).collect();
        files.sort_unstable();
        let mut expected = vec![CARGO_TOML, CARGO_TOML_PRE_VENDOR_FILTER, STUB_LIBRS];
        if with_orig {
            expected.push(CARGO_TOML_ORIG);
        }
        expected.sort_unstable();
        assert_eq!(files, expected);
        for (file, digest) in &checksums.files {
            let contents = std::fs::read(path.join(file)).unwrap();
            assert_eq!(&sha256_hexdigest(&contents).unwrap(), digest, "{file}");
        }
    }
}
